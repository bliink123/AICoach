...
def ai_coach():
    ...
    # Get recovery metrics from sleep data
    today = date.today().isoformat()
    sleep_data = garmin_client.get_sleep_data(today)
    ...
    # Check that we have all three metrics; if not, return an error message.
    ...
    # Build prompt for the AI coach
    prompt = (
        f"Based on an overall sleep score of {overall_value}, an average overnight HRV of {avg_over_night_hrv}, "
        f"and a body battery sleep increase of {body_battery_change}, using 300 characters - please recommend an appropriate workout for today."
    )

    # Retrieve Gemini API key and model from environment variables
    gemini_api_key = os.getenv("GEMINI_API_KEY")
    gemini_model = "gemini-2.0-flash-lite-preview-02-05"
    gemini_client = genai.Client(api_key=gemini_api_key)

    # Call the Gemini API to generate a recommendation
    try:
